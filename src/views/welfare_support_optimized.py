...
from src.utils.balances import ensure_member_balance_adjustments_table, get_member_welfare_reserve_balance
...
SELF_APPLICANT_ROLES = {"Chairperson", "Welfare", "Treasurer"}
...
def set_member_welfare_account_status(member_id: Optional[str], new_status: Optional[str], performed_by: str, actor_role: str, reason: Optional[str] = None) -> bool:
    ...
    execute_query(
        """
        INSERT INTO member_welfare_accounts (member_id, member_name, balance, created_at, status, updated_at)
        VALUES (%s, '', 0, CURRENT_TIMESTAMP, %s, CURRENT_TIMESTAMP)
        ON CONFLICT (member_id) DO NOTHING;
        """,
        params=(member_id, target_status),
        fetch=False,
    )
    execute_query(
        """
...
def validate_welfare_request_payload(
    ...
    # Allow specific leadership roles to self-apply even if their membership record
    # is not labeled as 'Full Member' (e.g., Chairperson, Welfare officer, Treasurer).
    actor_role = None
    try:
    ...
    if normalize_membership_status(membership_status) != "Full Member" and (actor_role not in SELF_APPLICANT_ROLES):
...
def _get_current_member_context(member_id: Optional[str]) -> dict:
    """OPTIMIZATION: Cache member-profile lookups for a short TTL."""
    if not member_id:
        return {}
    rows = execute_query(
...
def get_member_welfare_summary(member_id: Optional[str]) -> dict:
    """OPTIMIZATION: Combine the welfare summary query into one read-only fetch."""
    _ensure_welfare_tables()
    if not member_id:
        return {
            "welfare_balance": 0.0,
    ...
    contributions = float(row.get("contributions") or 0.0)

    return {
        "welfare_balance": round(get_member_welfare_reserve_balance(member_id), 2),
...
def get_unread_notification_count(user_role: Optional[str] = None, user_id: Optional[str] = None) -> int:
    """OPTIMIZATION: Cache unread notification counts for a short TTL."""
    ...
    return int((rows[0] or {}).get("unread_count") or 0)


def _is_full_member(member_context: dict) -> bool:
    if not member_context:
        return False
    return normalize_membership_status(member_context.get("status")) == "Full Member"
...
def _get_welfare_requests_for_role(user_role: str, member_id: Optional[str]) -> list[dict]:
    """OPTIMIZATION: Cache read-only request lists and filter in SQL."""
    _ensure_welfare_tables()
...
def render_welfare_support_page() -> None:
    _ensure_welfare_tables()
    ...
    member_context = _get_current_member_context(user_id)
    ...
    if is_staff_view:
        leader_metrics = get_welfare_leader_metrics()
        ...
        # Allow certain leadership roles to self-apply from the leadership view
        if user_role in SELF_APPLICANT_ROLES:
            with st.expander("Submit new support for yourself (Leadership)"):
                with st.form("welfare_self_apply_form"):
                    st.markdown("<div class='welfare-form-shell'>", unsafe_allow_html=True)
                    member_name = st.text_input("Member Name", value=member_context.get("full_name") or user_name, key="self_member_name")
                    membership_number = st.text_input("Membership Number", value=member_context.get("member_id") or "", key="self_membership_number")
                    phone_number = st.text_input("Phone Number", value=member_context.get("phone") or "", key="self_phone_number")
                    email_address = st.text_input("Email", value=member_context.get("email") or "", key="self_email_address")
                    membership_status = st.text_input("Membership Status", value=member_context.get("status") or "Full Member", key="self_membership_status")
                    date_joined = st.date_input("Date Joined", value=member_context.get("join_date") or datetime.utcnow().date(), min_value=date(1964, 1, 1), max_value=today_in_uganda(), key="self_date_joined")
                    request_date = st.date_input("Request Date", value=today_in_uganda(), min_value=date(2000, 1, 1), max_value=today_in_uganda(), key="self_request_date")
                    category_options = [_safe_str(row.get("category_name") or "General") for row in _get_welfare_categories()]
                    support_category = st.selectbox("Support Category", options=category_options, index=0 if category_options else None, key="self_support_category")
                    relationship = st.text_input("Relationship", value="", key="self_relationship")
                    event_date = st.date_input("Event Date", value=today_in_uganda(), min_value=date(2000, 1, 1), max_value=today_in_uganda(), key="self_event_date")
                    location = st.text_input("Location", value="", key="self_location")
                    description = st.text_area("Description", height=120, value="", key="self_description")
                    evidence_file = st.file_uploader("Supporting Evidence (optional)", type=["jpg", "jpeg", "png", "pdf"], help="Upload death announcement, funeral programme, invitation card, hospital letter, or other supporting evidence.", key="self_evidence_file")
                    st.markdown("</div>", unsafe_allow_html=True)
                    submitted_self = st.form_submit_button("Submit Welfare Request")
                ...
                if submitted_self:
                    ...
                    if request_data is not None:
                        st.success(f"Welfare request submitted. Case: {request_data.get('case_number')}")
                    else:
                        st.error("Could not submit the welfare request. Check logs and try again.")
        return
    ...
    with balance_col:
        with st.container(border=True):
            ...
            st.metric("Current balance", f"UGX {welfare_summary['welfare_balance']:,.0f}")
    ...
    with action_col:
        with st.container(border=True):
            ...
            if is_full_member or user_role in SELF_APPLICANT_ROLES:
    ...
    if not (is_full_member or user_role in SELF_APPLICANT_ROLES):
        st.warning("Only Full Members are eligible to request Welfare Support.")
        return

    categories = _get_welfare_categories()
    if not categories:
    ...
    with st.form("welfare_request_form"):
        ...
        member_name = st.text_input("Member Name", value=member_context.get("full_name") or user_name)
        membership_number = st.text_input("Membership Number", value=member_context.get("member_id") or "")
        phone_number = st.text_input("Phone Number", value=member_context.get("phone") or "")
        email_address = st.text_input("Email", value=member_context.get("email") or "")
        membership_status = st.text_input("Membership Status", value=member_context.get("status") or "Full Member")
        date_joined = st.date_input("Date Joined", value=member_context.get("join_date") or datetime.utcnow().date(), min_value=date(1964, 1, 1), max_value=today_in_uganda())
        request_date = st.date_input("Request Date", value=today_in_uganda(), min_value=date(2000, 1, 1), max_value=today_in_uganda())
        category_options = [_safe_str(row.get("category_name") or "General") for row in categories]
        support_category = st.selectbox("Support Category", options=category_options, index=0 if category_options else None)
        relationship = st.text_input("Relationship", value="")
        event_date = st.date_input("Event Date", value=today_in_uganda(), min_value=date(2000, 1, 1), max_value=today_in_uganda())
        location = st.text_input("Location", value="")
        description = st.text_area("Description", height=120, value="")
        evidence_file = st.file_uploader("Supporting Evidence (optional)", type=["jpg", "jpeg", "png", "pdf"], help="Upload death announcement, funeral programme, invitation card, hospital letter, or other supporting evidence.")
    ...
    if submitted:
        ...
        if request_data is not None:
            st.success(f"Your welfare request has been submitted successfully. Case number: {request_data.get('case_number')}")
        else:
            st.error("We could not submit your request. Please try again later.")

    st.markdown("<div class='welfare-panel-title' style='margin-top: 18px;'>Your welfare history</div>", unsafe_allow_html=True)
    member_requests = _get_welfare_requests_for_role("Member", user_id)
